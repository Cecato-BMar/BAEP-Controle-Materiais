...
def api_lotes(request):
    try:
        ...
        logger.error('Erro em api_lotes: %s', e, exc_info=True)
        return JsonResponse({'error': 'Erro interno ao carregar lotes.'}, status=500)
...
def api_retiradas_pendentes(request):
    try:
        ...
        for retirada in retiradas:
            ...
            if pendente > 0:
                data.append({
                    ...
                    'numero': getattr(retirada.material, 'numero', None),
        ...
        logger.error('Erro em api_retiradas_pendentes: %s', e, exc_info=True)
        return JsonResponse({'error': 'Erro interno ao carregar retiradas pendentes.'}, status=500)
...
def api_retirada_detalhe(request, retirada_id):
    try:
        ...
        logger.error('Erro em api_retirada_detalhe (id=%s): %s', retirada_id, e, exc_info=True)
        return JsonResponse({'error': 'Erro interno ao carregar detalhes da retirada.'}, status=500)
